...
Class Master extends DBConnection {
	...
	function save_brand(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Numele de brand există deja!";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "Noul brand a fost salvat!";
			else
				$resp['msg'] = "Brand actualizat cu succes!";
			if(!empty($_FILES['img']['tmp_name'])){
				...
				if(!in_array($_FILES['img']['type'],$accept)){
					$resp['msg'] .= " Tipul imaginii este nevalid.";
				}
				...
				if(!$uploadfile){
					$resp['msg'] .= " Imaginea este nevalidă.";
				}
	...
	function delete_brand(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Brand șters cu succes.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_category(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Această categorie există deja!";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"Noua categorie a fost creată cu succes.");
			else
				$this->settings->set_flashdata('success',"Categoria a fost actualizată cu succes.");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_category(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Categoria a fost ștearsă cu succes.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_product(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Acest produs există deja.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			...
			if(isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0){
				...
				foreach($_FILES['img']['tmp_name'] as $k => $v){
					if(!empty($_FILES['img']['tmp_name'][$k])){
						...
						if(!in_array($_FILES['img']['type'][$k],$accept)){
							$err = "Tipul imaginii este nevalid.";
							break;
						}
				...
				if(!empty($err)){
					$resp['status'] = 'failed';
					$resp['msg'] = 'Produsul s-a salvat cu succes, dar '.$err;
					$resp['id'] = $pid;
				}
			}
			if(!isset($resp)){
				$resp['status'] = 'success';
				if(empty($id))
					$this->settings->set_flashdata('success',"Produsul nou a fost salvat cu succes.");
				else
					$this->settings->set_flashdata('success',"Produsul a fost actualizat cu succes.");
			}
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
	...
	function delete_product(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Produsul a fost șters cu succes.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_inventory(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Inventarul există deja";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"Inventarul nou a fost salvat cu succes.");
			else
				$this->settings->set_flashdata('success',"Inventarul a fost actualizat cu succes.");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_inventory(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Inventarul a fost șters cu succes.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function register(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Acest email este folosit deja!";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"Cont creat cu succes.");
			else
				$this->settings->set_flashdata('success',"Cont actualizat cu succes.");
	...
	function delete_order(){
		...
		if($delete){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Comanda a fost ștearsă cu succes!");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function place_order()
	{
		...
		if ($this->capture_err())
			return json_encode(['status' => 'failed', 'error' => $this->capture_err()]);
		if ($save_order) {
			...
			// Debug: Check the values of $list_sql
			error_log("SQL for order_list: " . $list_sql);
			...
			if ($save_olist) {
				...
				if ($this->capture_err())
					return json_encode(['status' => 'failed', 'error' => $this->capture_err()]);
				$resp['status'] = 'success';
				$this->settings->set_flashdata('success', "Comanda a fost plasată cu succes!");
			} else {
				$resp['status'] = 'failed';
	...
	function update_order_status(){
		...
		if($update){
			$resp['status'] ='success';
			$this->settings->set_flashdata("success","Starea comenzii a fost actualizată cu succes.");
		}else{
			$resp['status'] ='failed';
	...
	function pay_order(){
		...
		if($update){
			$resp['status'] ='success';
			$this->settings->set_flashdata("success"," Starea plății comenzii a fost actualizată cu succes.");
		}else{
			$resp['status'] ='failed';
			$resp['err'] =$this->conn->error;
		}
	...
	function update_account(){
		...
		if(md5($cpassword) != $this->settings->userdata('password')){
			$resp['status'] = 'failed';
			$resp['msg'] = "Parola curentă este invalidă!";
			return json_encode($resp);
			exit;
		}
		...
		if($save){
			...
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success','Detaliile contului dvs. au fost actualizate cu succes.');
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function update_client(){
		...
		if($save){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success','Detaliile clientului au fost actualizate cu succes.');
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function delete_client(){
		...
		if($delete){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Client șters cu succes.");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error;
		}
