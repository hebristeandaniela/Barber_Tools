...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				...
				$this->settings->set_flashdata('success','Detaliile utilizatorului au fost salvate cu succes.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
				...
				if(!empty($_FILES['img']['tmp_name'])){
					...
					if(!in_array($_FILES['img']['type'],$accept)){
						$err = "Tipul fișierului imagine este nevalid";
					}
			...
			if($qry){
				$this->settings->set_flashdata('success','Detaliile utilizatorului au fost actualizate cu succes.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
						$this->settings->set_userdata($k,$v);
	...
	public function delete_users(){
		...
		if($qry){
			$this->settings->set_flashdata('success','Detaliile utilizatorului au fost șterse cu succes.');
			return 1;
		}else{
			return false;
		}
